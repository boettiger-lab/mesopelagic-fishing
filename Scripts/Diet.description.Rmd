...
predators_mesopelagics <- as.data.frame(predators_mesopelagic) %>% mutate(classification= "predators") %>% rename(spp= predators_mesopelagic)
...
mesopelagics_only <- data_2 %>% filter(spp== "mesopelagics")
ggplot() + geom_line(data=data_2_edt, aes(x=month, y=agg_rel_biomass, col= as.factor(classification))) + geom_line(data=mesopelagics_only, aes(x = month, y = biomass), color = "blue") + 
