...
#Graphs 
plot3 <- data_3 %>% filter(spp == c("Market squid", "Sardine", "Anchovy", "Herring", "Pacific Mackerel", "Sand lance", "Whitebait smelt", "Other smelt", "Saury")) %>% ggplot(aes(x= month, y = biomass)) + geom_line(aes(colour= factor(spp))) + xlab("Month") + ylab("Biomass") + 
...
#test <- ggplot(change_3_df, aes(x=value, y=species, label=species)) + 
...
```{r}
target_sp <- c("mesopelagics", "euphausiids", "small jellies", "large jellies", "carniv-zoops")
target_F <- c(1.0, 0.04, 0.04, 0.04, 0.04)
names(target_F) <- target_sp 
