...
plot5_6 <- data_5 %>% filter(spp %in% high.price) %>% ggplot(aes(x= month, y = biomass)) + geom_line(aes(colour= factor(spp))) + xlab("Month") + ylab("Relative biomass") + 
theme_bw() + 
scale_color_viridis(discrete = TRUE, option = "inferno")+
theme(panel.border = element_rect(fill=NA, colour = "black", size=1)) + 
theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + labs(colour = "Predators with highest economic value") + ylim(0.95, 1.15) + 
ggsave(width = 9, height = 6, "~/Documents/Thesis/Images/50/plot.highprice.50.jpeg")
plot5_6

plot5_7 <- data_5 %>% filter(spp %in% mammals) %>% ggplot(aes(x= month, y = biomass)) + geom_line(aes(colour= factor(spp))) + xlab("Month") + ylab("Relative biomass") + 
theme_bw() + 
scale_color_viridis(discrete = TRUE, option = "inferno")+
theme(panel.border = element_rect(fill=NA, colour = "black", size=1)) + 
theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + labs(colour = "Protected predators of mesopelagic fish") + ylim(0.9, 1.15) + 
...
shapiro.test(change_5_1$value)
shapiro.test(change_5_2$value)
shapiro.test(change_5_3$value)
shapiro.test(change_5_4$value)
shapiro.test(change_5_5$value)
shapiro.test(change_5_7$value)
shapiro.test(change_5_8$value)

shapiro.test(change_6_1$value)
shapiro.test(change_6_2$value)
shapiro.test(change_6_3$value)
shapiro.test(change_6_4$value)
shapiro.test(change_6_5$value)
shapiro.test(change_6_6$value)
shapiro.test(change_6_7$value)

#Normally distributed data: p > 0.05

shapiro.test(change_5_6$value)
shapiro.test(change_5_9$value)

shapiro.test(change_6_8$value)
shapiro.test(change_6_9$value)
