...
change_3_df<- change_3_overtime_df(diet_over_0.05)
change_3_df <- as.data.frame(change_3_df)
change_3_final <- change_3_df %>% mutate(value = round(as.numeric(paste(value)), 2)) %>% arrange(value)

nb.cols <- 12

try <- change_3_final %>% ggplot(aes(x= value, y=species)) + geom_point(aes(col= species)) + xlim(0.2, 1.6) + geom_vline(xintercept=1, linetype= "solid", color= "black", size= 0.75) + scale_color_viridis(discrete = TRUE, option = "D")+
...
plot <- data_4 %>% filter(spp == c("Market squid", "Sardine", "Anchovy", "Herring", "Pacific Mackerel", "Sand lance", "Whitebait smelt", "Other smelt", "Saury")) %>% ggplot(aes(x= month, y = biomass)) + geom_line(aes(colour= factor(spp))) + xlab("Month") + ylab("Biomass") + 
...
plot1 <- data_5 %>% filter(spp == c("Market squid", "Sardine", "Anchovy", "Herring", "Pacific Mackerel", "Sand lance", "Whitebait smelt", "Other smelt", "Saury")) %>% ggplot(aes(x= month, y = biomass)) + geom_line(aes(colour= factor(spp))) + xlab("Month") + ylab("Biomass") + 
scale_colour_brewer(palette = "Paired") +
theme_bw() + 
theme(panel.border = element_rect(fill=NA, colour = "black", size=1)) + 
theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + labs(colour = "Forage fish")
...
plot3 <- data_6 %>% filter(spp == c("Market squid", "Sardine", "Anchovy", "Herring", "Pacific Mackerel", "Sand lance", "Whitebait smelt", "Other smelt", "Saury")) %>% ggplot(aes(x= month, y = biomass)) + geom_line(aes(colour= factor(spp))) + xlab("Month") + ylab("Biomass") + 
theme_bw() + 
theme(panel.border = element_rect(fill=NA, colour = "black", size=1)) + 
theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + labs(colour = "Forage fish")
plot3
...
change_3_df<- change_overtime_df(diet_over_0.05)
change_3_df <- as.data.frame(change_3_df)
change_3_final <- change_3_df %>% mutate(value = round(as.numeric(paste(value)), 2)) %>% arrange(value)

try <- change_3_final %>% ggplot(aes(x= value, y=species)) + geom_point(aes(col= species)) + xlim(0.8,1.2) + geom_vline(xintercept=1, linetype= "solid", color= "black", size= 1.2)
try
...
target_F <- c(1.0, 0.2, 0.2, 0.2, 0.2)
names(target_F) <- target_sp 
...
for (yr in 2001:2020){
  ...
  assessment <- end_biomass(run3.2)[target_sp] * (runif(5) + 0.5)   
#assessment <- end_biomass(run5)[target_sp] * (runif(length(target_sp)) + 0.5)
assessment
  # convert target F * assessment to a total catch 
  catches <- target_F * assessment
