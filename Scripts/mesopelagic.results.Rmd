...
#Changing data to group functional groups
```{r}
test1 <- data_6 %>% filter(spp %in% noncommerical_predators, month==1) %>% summarize(avg= mean(biomass))
test1

data_5 %>% left_join(FG))
data_5 %>% group_by(FG) %>% summarize(biomass = mean(biomass))
