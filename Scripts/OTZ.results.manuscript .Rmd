...
plot1_comm <- data_1 %>%
  filter(spp %in% commercial_predators) %>% mutate(spp = fct_reorder(spp, biomass, tail, n = 1, .desc = TRUE)) %>% ggplot(aes(x= month, y = biomass)) + geom_line(aes(colour= spp)) + xlab("Month") + ylab("Relative biomass") +
...
direct.label(plot1_comm$spp, list("last.points", cex=.7, hjust=1))


+ geom_dl(aes(label= spp), method= list(dl.trans(x= x - 2.0), "last.points", cex= 0.8))
