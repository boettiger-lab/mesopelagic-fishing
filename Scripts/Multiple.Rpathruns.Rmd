...
```{r}
  ...
  #Edit the datasets
  CalCur_unbal[[i]] <- read.rpath.params(Cbase, Cdiet)
  CalCur_unbal[[i]][[1]] <- CalCur_unbal[[i]][[1]] %>%
    select(-X)
  CalCur_unbal[[i]][[1]][93, 5] <- NA
  CalCur_unbal[[i]][[1]][CalCur_unbal[[i]][[1]] == -1] <- NA
   CalCur_unbal[[i]][[2]] <- CalCur_unbal[[i]][[2]] %>%
    select(-X)

  check.rpath.params(CalCur_unbal[[i]])
}
