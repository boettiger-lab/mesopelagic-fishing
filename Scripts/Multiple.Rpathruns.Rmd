...
table_Biomass <- bind_rows(rest_of_models) %>% group_by(Group) %>% summarize(IQR_biomass= IQR(Biomass), quantile_lower = quantile(Biomass, 0.025), quantile_upper = quantile(Biomass, 0.95)) %>% mutate(lower_outlier = quantile_lower - (1.5*IQR_biomass), upper_outlier= quantile_upper + (1.5*IQR_biomass)) 
...
```{r}
seven_models_outliers <- bind_rows(data_6.2) %>% filter(X %in% c(67, 126, 237, 343, 396, 430, 472)) %>% arrange(spp)
seven_models_outliers <- transform(seven_models_outliers,ID=as.numeric(factor(spp))) 
seven_models_outliers <- seven_models_outliers %>% group_by(spp, ID) %>% summarise(mean_biomass = mean(biomass))

#Need to calculate IQR of rest_of_model_outliers before we find mean_biomass
rest_of_models_outliers <- sum_data_6 %>% summarize(IQR_biomass = IQR(Biomass)) %>% select(spp, mean_biomass, quant_2.5, quant_97.5, IQR_biomass)

table_Biomass <- bind_rows(rest_of_models) %>% group_by(Group) %>% summarize(IQR_biomass= IQR(Biomass), quantile_lower = quantile(Biomass, 0.025), quantile_upper = quantile(Biomass, 0.975)) %>% mutate(lower_outlier = quantile_lower - (1.5*IQR_biomass), upper_outlier= quantile_upper + (1.5*IQR_biomass)) 
