...
legend <- get_legend(comm_value_500)
pdf("/Users/SallyDowd/Documents/OTZ.manuscript/Images/Legends/multiple.legend.pdf", width= 6, height= 5)
grid.draw(legend)
dev.off()
...
##Biomass- keep this for now  
```{r}
seven_models_outliers <- CalCur_unbal2_list[c(67, 126, 237, 343, 396, 430, 472)]
rest_of_models_outliers <- CalCur_unbal2_list[-c(67, 126, 237, 343, 396, 430, 472)]

rest_of_models

for (i in 1:493) {
rest_of_models_outliers[[i]] <- rest_of_models_outliers[[i]]$model %>% dplyr::select(Biomass, QB, PB, EE, Group) 
}

table_Biomass <- bind_rows(rest_of_models_outliers) %>% group_by(spp) %>% summarize(IQR_biomass= IQR(biomass), quantile_lower = quantile(biomass, 0.025), quantile_upper = quantile(biomass, 0.95)) %>% mutate(lower_outlier = quantile_lower - (1.5*IQR_biomass), upper_outlier= quantile_upper + (1.5*IQR_biomass)) 
...
#Are the results outliers
```{r}
df
letters <- c("A", "B", "C", "D", "E", "F", "G")
X <- c(67, 126, 237, 343, 396, 430, 472)
letters <- letters %>% bind_cols(X) 
seven_models_outliers <- bind_rows(data_6.2) %>% filter(X %in% c(67, 126, 237, 343, 396, 430, 472)) %>% arrange(X)
seven_models_outliers <- seven_models_outliers %>% mutate(ID.X = revalue(X, c(67="A", 126="B", 237= "C", 343= "D", 396= "E", 430= "F", 472= "G")))

seven_models_outliers <- bind_rows(data_6.2) %>% filter(X %in% c(67, 126, 237, 343, 396, 430, 472)) %>% arrange(X)
seven_models_outliers <- transform(seven_models_outliers, ID.X=as.numeric(factor(X)))  
seven_models_outliers <- seven_models_outliers %>% mutate(ID.X = revalue(c(67, 126, 237, 343, 396, 430, 472), c("A", "B", "C", "D", "E", "F", "G"))

  X, c(67="A", 126="B", 237= "C", 343= "D", 396= "E", 430= "F", 472= "G")))
  mutate(ID.X= as.character(X))
...
seven_models_outliers <- seven_models_outliers %>% spread(ID.X, biomass) 

test <- seven_models_outliers %>% left_join(rest_of_models_outliers) %>%
  mutate(outlier1 = ifelse(1 < lower_outlier|1 > upper_outlier, TRUE, FALSE)) 
...
outlier <- list()
for (i in unique(seven_models_outliers$ID.X)){
subdf <- subset(x=seven_models_outliers, subset=ID.X==i)
outlier[i] <- subdf %>% group_by(Group) %>% left_join(rest_of_models_outliers) %>% mutate(outlier= ifelse(biomass < lower_outlier|biomass > upper_outlier, TRUE, FALSE)) %>% select(outlier)
}
