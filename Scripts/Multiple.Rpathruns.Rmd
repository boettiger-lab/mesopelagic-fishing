...
seven_models_outliers <- transform(seven_models_outliers,ID=as.numeric(factor(spp))) %>% group_by(X)
...
outlier <- list()
for (i in 1:7) {
seven_out <- seven_model_outliers %>% dplyr::select(Biomass, QB, PB, EE, Group) %>% filter(Group== "mesopelagics")
outlier[i] <- seven_models_outliers %>% left_join(rest_of_models_outliers) %>% mutate(outlier= ifelse(Biomass < lower_outlier|Biomass > upper_outlier, TRUE, FALSE)) %>% select(outlier)
}
outlier[1]
