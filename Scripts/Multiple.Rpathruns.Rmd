...
#Creating empty lists, defining year sequence
scene6.2 <- list()
run6.2 <- list()
assessment2 <- list()
catches2 <- list()
...
run6.2[[i]] <- rsim.run(scene6.2[[i]], method = 'AB', years=2001) #When running a multistep run your initial run is up to the point of your first break
#Stock assessment based on end biomass from run6 year 2001, will get 500 different assessments 
assessment2[[i]] <- end_biomass(run6.2[[i]])[target_sp] * (runif(length(target_sp)) + 0.5) 
assessment2[[i]]
#Find total catch in year 2001 based off stock assessment, 500 different catches 
  catches2[[i]] <- target_F * assessment2[[i]]
#Apply the catch to the target species, considering catch from first year of the run 
#ForcedCatch: catch added directly to loss due to fishing, input with annual resolution, overrides dynamics from fleets and doesn't change bycatch 
scene6.2[[i]] <- adjust.fishing(scene6.2[[i]], "ForcedCatch", target_sp, year[[j]], value= catches2[[i]][target_sp])
#Rsim.step runs rsim.run at intervals and combines the run to one ouptput, Pause operating model to extract data, these results can be used to adjust parameters of operating model before next iteration
#Run 1 year
  run6.2[[i]] <- rsim.step(scene6.2[[i]], run6.2[[i]], method = 'AB', year.end= year[[j]])  #year.end: end year for current interval
}  
}
...
data_6.2[[2]]
#In format: month, species, biomass, want 95% CI for each model at month 600 for each commercially and noncommercially valuable species 
