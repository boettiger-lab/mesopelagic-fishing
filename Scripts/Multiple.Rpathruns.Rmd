...
#Want 95% CI for each model at month 600 for each commercially and noncommercially valuable species 
#1) Combine datasets
#2) Check to see if 500 end biomass values for each species are normally distributed 
#1) check if 500 end biomass values for each species are normally distributed

entire_data_6 <- bind_rows(data_6.2) %>% arrange(spp) %>% filter(!X %in% c(67, 126, 237, 343, 396, 430, 472), spp == "Dolphins")
shapiro.test(entire_data_6$biomass)

(!Income_Group %in% c("High income: OECD", "High income: nonOECD"))
entire_data_6 <- bind_rows(data_6.2) %>% arrange(spp) %>% filter(spp == "mesopelagics")
shapiro.test(entire_data_6$biomass)
