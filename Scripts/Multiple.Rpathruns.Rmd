...
seven_out <- seven_models_outliers$ID.X[[i]]
outlier[i] <- seven_out %>% left_join(rest_of_models_outliers) %>% mutate(outlier= ifelse(biomass < lower_outlier|biomass > upper_outlier, TRUE, FALSE)) %>% select(outlier)
}
```
