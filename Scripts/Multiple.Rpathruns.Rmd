...
for (i in 1:500) {
for(yr in 2002:2050){
scence6.2[[i]] <- copy(base_sim_scene2[[i]])
run6.2[[i]] <- rsim.run(scence6.2[[i]], method = 'AB', years=2001)
assessment <- end_biomass(run6.2[[i]])[target_sp] * (runif(length(target_sp)) + 0.5) #argument of length 0 means input is of length zero 
assessment
  # convert target F * assessment to a total catch 
  catches <- target_F * assessment
  # apply each catch to each species
  for (sp in target_sp){
    scene6.2[[i]] <- adjust.fishing(scene6.2[[i]], "ForcedCatch", sp, yr, value=catches[sp])
  }
  # run 1 year
  run6.2[[i]] <- rsim.step(scene6.2[[i]], run6.2, method = 'AB', year.end=yr)  
}  
}
```
