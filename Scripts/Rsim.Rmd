...
REco.sim <- adjust.fishing(REco.sim, 'EFFORT', group = 'Trawlers', sim.year = 25:100, 
                           value = 2) #In the new version of Rpath (uploaded to NOAA github, it is 'ForcedEffort' instead of just effort)
```
...
```{r rsim.run}
REco.run1 <- rsim.run(REco.sim, method = 'AB', years = 1:100) #Also in new Rpath the method used is 'RK4' instead of 'AB'
```
