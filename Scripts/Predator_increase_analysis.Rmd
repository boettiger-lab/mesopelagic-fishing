...
#Scatter plot to look at diet dependence of predators of mesopelagic fish vs. end biomass. This code generates Figure ___ in Appendix 2. 
```{r}
...
predators_endbiomass %>% ggplot(aes(x= value, y= endbiomass)) + geom_point() + xlab("Diet proportion on mesopelagics") + ylab("Relative end biomass") + standard_theme + ggsave(width = 8.75, height = 5.25, "/Users/sallydowd/Documents/OTZ.manuscript/Images/Submission.2/endbiomass.dietvalue.jpeg", dpi=600)

library(broom)
 mylm <- lm(endbiomass ~ value, data = predators_endbiomass)
summary(mylm)
model.diag.metrics <- augment(mylm)
head(model.diag.metrics)
par(mfrow = c(2, 2))
plot(mylm)

model2 <- lm(log(endbiomass) ~ value, data = predators_endbiomass)
plot(model2, 3)

shapiro.test(predators_endbiomass$endbiomass)
